...
def Reach2Nodes(SHP, EPSG, toll, storedir):
    ...
    END_out = SHP.CreateDataSource(END_SHP)
    ...
    START_out = SHP.CreateDataSource(START_SHP)
    ...
    CONN_out = SHP.CreateDataSource(CONN_SHP)
...
def ReachOrder(SHP, EPSG, toll, storedir):
    ...
    if os.path.exists(ORDER_SHP):
        SHP.DeleteDataSource(ORDER_SHP)
    ...
    for i in range(1, max(ReachOrders) + 1):
        ...
        if os.path.exists(ORDER_SHP):
            SHP.DeleteDataSource(ORDER_SHP)
        ORDER_out = SHP.CreateDataSource(ORDER_SHP)
...
def Tiff2Point(TIFF):
    ...
    if os.path.exists(SHP):
        SHP.DeleteDataSource(SHP)

    SHP_out = SHP.CreateDataSource(SHP)
